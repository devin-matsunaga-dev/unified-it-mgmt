...
import { PriorityPill, StatusPill, formatLocal, formatRemaining, slaIcon } from './ticketUi'

const workflow = ['New', 'Triage', 'InProgress', 'Pending', 'Resolved', 'Closed'] as const
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]">
      <div className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="border-b border-slate-200 p-5 dark:border-slate-800"><h2 className="font-semibold">Activity</h2><p className="mt-1 text-sm text-slate-500">Comments, internal notes, status changes, and assignments.</p></div>
          <div className="p-5"><form onSubmit={(event) => { event.preventDefault(); if (comment.trim()) addComment.mutate() }}><textarea aria-label="Comment" className="input min-h-24 resize-y" placeholder={internal ? 'Add an internal note…' : 'Write a public reply…'} value={comment} onChange={(event) => setComment(event.target.value)} /><div className="mt-3 flex flex-wrap items-center gap-3"><label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300"><input type="checkbox" checked={internal} onChange={(event) => setInternal(event.target.checked)} />Internal note</label>{addComment.error && <span role="alert" className="text-sm text-red-600">{addComment.error.message}</span>}<Button className="ml-auto" type="submit" disabled={!comment.trim() || addComment.isPending}><Send size={16} />{internal ? 'Add note' : 'Reply'}</Button></div></form></div>
      ...
      <aside className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><h2 className="font-semibold">Ticket details</h2><dl className="mt-4 space-y-3 text-sm"><Detail label="Type" value={item.type === 'Incident' ? 'Incident' : 'Service request'} /><Detail label="Urgency" value={item.urgency} /><Detail label="Impact" value={item.impact} /><Detail label="Updated" value={formatLocal(item.updatedAt)} /></dl></section>
      </aside>
    </div>
  </div>
}
...
function displayStatus(status: string) { return status === 'InProgress' ? 'In progress' : status }
