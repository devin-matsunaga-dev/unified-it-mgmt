...
export function TicketListPage() {
  ...
  return <div className="space-y-6">
    ...
    <QuickCreateTicket open={createOpen} pending={create.isPending} error={create.error instanceof Error ? create.error.message : undefined} queues={queues.data ?? []} onClose={() => { if (!create.isPending) { setCreateOpen(false); create.reset() } }} onSubmit={async (input) => { await create.mutateAsync(input) }} />
  </div>
}
