...
import type { CreateTicketInput, TicketQueue } from '../../api/helpdesk'
...
export function QuickCreateTicket({ open, pending, error, queues, onClose, onSubmit }: { open: boolean; pending: boolean; error?: string; queues: TicketQueue[]; onClose: () => void; onSubmit: (input: CreateTicketInput) => Promise<void> }) {
  ...
  const submit = handleSubmit(async (values) => {
    await onSubmit({ ...values, requesterId: values.requesterId || null, queueId: values.queueId || null })
    reset()
  })
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section role="dialog" aria-modal="true" aria-labelledby="create-ticket-title" className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
