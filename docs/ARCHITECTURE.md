...
- When one question has to be asked of **every** module at once, the shape is a **contribution interface**, not a port per module: the contract lives in `Platform`, each module registers its own implementation over its own schema beside its services, and a Platform service is handed all of them by the container and composes the answers (`ISearchSource` → global search, WP-5.4; `IDashboardWidget` → the unified dashboard, WP-5.5). No module is reached into and Platform still references nothing, so adding a module's contribution is a registration. Use this only where the question is genuinely estate-wide and read-only; two modules reading each other is still a port.
