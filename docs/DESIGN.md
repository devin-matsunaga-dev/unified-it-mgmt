...
- **Field surface** (`/field`, technicians on a phone): the one area exempt from the 1280px floor in §10, because it exists for a handset. Same tokens, no dark sidebar, no tables. Single column, full-bleed cards at 16px page padding, `max-w-[520px]` centred so a tablet does not stretch a line of text. **Touch targets are 48px minimum** (not the 36-40px of §6) and primary actions are **anchored to the bottom of the viewport**, within thumb reach, never the top. Type goes up one step from §4: body 16px, page title 22px/700 — 14px is unreadable in daylight on a loading dock. One screen answers one question — **except a stock count, which is the one screen built to be used continuously**: the camera stays live, results stack under the viewfinder, and nothing navigates until the technician stops.  **A handset cannot reach the agent shell at all** — every agent route redirects to the field surface below 768px, with no escape hatch, because the agent screens are dense enough that a mis-tap is a write to the CMDB. **The consequence is binding on anyone extending this: whatever a technician needs in the field has to exist here, because they cannot route around a gap.**
