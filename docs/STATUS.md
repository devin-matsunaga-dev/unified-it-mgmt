...
- **Last completed WP:** WP-3.2 (Python poller skeleton + heartbeat) — automated tests green (483 .NET solution-wide, plus 30 pytest in `services/poller`, ruff and mypy clean); manual checklist walked against a live `aspire run` on 2026-08-10, 10/10 checks. **The walk found two real defects, both fixed and both now covered by tests** — see In flight
- **Current WP:** WP-3.3 (ICMP + SNMP polling)
- **Current branch:** `feat/wp-3.3-icmp-snmp-polling`
- **After it:** WP-3.4 (Metrics storage, TimescaleDB)
