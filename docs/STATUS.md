...
- **Last completed WP:** WP-3.3 (ICMP + SNMP polling) — automated tests green (502 .NET solution-wide, plus 109 pytest in `services/poller`, ruff and mypy clean); manual checklist walked against a live `aspire run` on 2026-08-10, 12/12 checks. **The walk found one real defect — the simulator's address — which is fixed and covered; see In flight**
- **Current WP:** WP-3.4 (Metrics storage, TimescaleDB)
- **Current branch:** `feat/wp-3.4-metrics-storage`
- **After it:** WP-3.5 (Alert engine, state machine)
...
- **Telemetry has no consumer, by design — "metrics events flow" is verified at the broker, not in a database.** WP-3.4 owns ingestion and storage. Until then `Contracts.Events:DeviceTelemetryReported` and `Contracts.Events:DeviceReachabilityChanged` are fanout exchanges with nothing bound to them, so messages published there are discarded by RabbitMQ. Watch the publish rate in the management UI, or bind a temporary queue. **A consumer added in WP-3.4 will start receiving immediately with no poller change.**
...
- **`PollerHeartbeatMissed` is a fact, not an alert.** Nothing consumes it yet. WP-3.5's state machine and WP-3.6's ticket automation are its intended consumers; the evaluator deliberately does not raise an alert, open a ticket or notify anyone. What it does do is write an audit entry (`Poller` / `HeartbeatMissed`), which is also what flushes the outbox.
