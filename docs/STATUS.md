...
- **Last completed WP:** WP-3.4 (Metrics storage, TimescaleDB) — automated tests green (529 .NET solution-wide, up from 502); the poller is unchanged, so its 109 pytest, ruff and mypy results stand from WP-3.3. Manual checklist walked against a live `aspire run` on 2026-08-10, 11/11 checks. **The walk found one real defect — a series over a metric name several checks report — which is fixed and covered; see In flight**
- **Current WP:** WP-3.5 (Alert engine, state machine) **[SENSITIVE — core logic]**
- **Current branch:** `feat/wp-3.5-alert-state-machine`
- **After it:** WP-3.6 (Alert→ticket automation)
