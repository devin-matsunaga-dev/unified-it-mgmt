...
### WP-2.10 — Mixed-type import
- A "Mixed — read from a column" choice beside the six CI types in the WP-2.5 wizard: `type` becomes a mappable target, the mapping step offers the union of every type's attributes (a row reads only the ones its own type declares, ignoring the rest), and required-ness is shown per type. Where no type column exists, infer each row's type from its unique discriminator (`manufacturer`/`model` → Hardware, `operatingSystem`/`cpuCores` → Server, `managementIp`/`portCount` → NetworkDevice, `version` → Software, `hypervisor`/`vcpuCores` → Virtual, `purpose`/`serviceTier` → Logical) and show the guess as a column in the dry run — **never commit an inferred type the operator has not seen**. Single-type import stays the default and is untouched.
...
- **Scope — to be listed before implementation starts:**
  - _(1)_ **Rebuild the CI detail page's Relations card for legibility.** Three labelled sections in place of the flat edge list plus hop bands: *Direct relationships* (the CI's own edges as cards, grouped by relationship type with a per-group count, carrying type, lifecycle and description, actions behind a `⋮` menu); *Dependency path* (what this CI relies on, drawn as an indented tree so the reader can see that a CI two hops out is reached *through* the one at one hop, with each connector labelled by its relationship verb); *Downstream impact* (the same tree walked the other way, with its own empty state). Each tree states how many levels it is showing and offers **Show deeper** up to the server's 10-hop ceiling, replacing the hardcoded 3. The header count becomes the number of *direct* relationships rather than every edge in the traversal.
  - _(2)_ **Give the asset list's empty and error states a dark-mode palette.** Their icon circles were the only tinted circles in the module with no `dark:` variant, so both rendered as a near-white disc on a slate-900 card (DESIGN.md §8).
  - _(3)_ **Make the CI page's two history cards distinguish "nothing happened" from "could not be read".** Lifecycle history and the check-in/out log answered a failed request with their empty sentence, asserting a fact about the asset that nobody had established. Both now get a skeleton while loading and a stated failure, matching every other card on the page (DESIGN.md §10).
  - _(4)_ **State the coverage dates as calendar dates.** Contract end, purchase and warranty dates were the only raw `yyyy-MM-dd` strings on a page of formatted ones. They are `DateOnly` by WP-2.6's decision, so they are formatted by parts rather than parsed as instants — `new Date('2026-09-14')` is UTC midnight, which renders a day early west of Greenwich.
  - _(5)_ **Add the KPI row DESIGN.md §5 specifies to `/assets`.** Four tiles — total, Deployed, In repair, and warranties ending within 30 days — each counted by the list endpoint under the filter it represents, and each clickable to narrow the list to exactly what it counts. Tile tints follow the lifecycle pills in the table below; the total stays neutral because a count is not a status (§3). No "since last week" delta: nothing records a historical count, so there is no honest one to show.
  - _(6)_ **Sortable columns on the asset table, matching the ticket list.** Every column header cycles ascending → descending → the server's own order, with the same arrow affordance. Lifecycle sorts by its place in the WP-2.2 state graph rather than alphabetically. Like the ticket list this sorts in the browser, but unlike it `/assets` pages at 25 while `/api/cis` orders by name and takes no sort parameter — so a sort covers the page on screen and the footer says so whenever more than one page exists. Cross-page sorting needs a `sort` parameter on `/api/cis`; that is a backend change and is **not** in this package.
